...
extern void traceMoveAfter(InfomapBase& base, InfoNode& current,
...
void InfomapOptimizer<Objective>::initPartition()
{
  ...
  // [TRACE-IM] snapshot the active network's per-node flow state +
  // initial codelength so the JS replay can rebuild this level.
  // is_main passed from here because InfomapOptimizer is friend of
  // InfomapBase + the free-function traceBeginLevel is not.
  traceBeginLevel(*m_infomap, m_infomap->isMainInfomap());
}
...
void InfomapOptimizer<Objective>::moveActiveNodesToPredefinedModules(std::vector<unsigned int>& modules)
{
  ...
  // [TRACE-IM] capture the predefined-modules vector for the current
  // level before applying any moves; JS replay applies the same vector
  // at level start (after singleton init) before the first tryMoveEach.
  tracePredefinedModules(*m_infomap, modules);
...
bool InfomapOptimizer<Objective>::moveNodeToPredefinedModule(InfoNode& current, unsigned int newModule)
{
  ...
  current.index = newM;
  // [TRACE-IM] log the predefined-modules move (used by fineTune,
  // coarseTune, restoreHardPartition, etc.).
  traceMoveAfter(*m_infomap, current, oldM, newM,
                 oldModuleDelta.deltaEnter, oldModuleDelta.deltaExit,
                 newModuleDelta.deltaEnter, newModuleDelta.deltaExit);
  return true;
}
...
unsigned int InfomapOptimizer<Objective>::tryMoveEachNodeIntoBestModule()
{
  ...
  for (unsigned int i = 0; i < numNodes; ++i) {
    ...
    if (bestDeltaModule.module != current.index) {
      ...
      // [TRACE-IM] log the move (oldModuleDelta + bestDeltaModule are
      // exactly what canonical's MapEquation::updateCodelengthOnMovingNode
      // received; JS replay applies the same delta values via the same
      // closed-form ΔL formula).
      traceMoveAfter(*m_infomap, current, oldModuleIndex, bestModuleIndex,
                     oldModuleDelta.deltaEnter, oldModuleDelta.deltaExit,
                     bestDeltaModule.deltaEnter, bestDeltaModule.deltaExit);

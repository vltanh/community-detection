/******************************************************************************
 * push_relabel.h (instrumented sibling)
 *
 * Source of VieCut (Adapted from KaHIP) — clone of
 * VieCut/lib/algorithms/flow/push_relabel.h with [TRACE-PR] stderr probes
 * at tie-break decision sites T16 (push.h:148 sourceDistance<=distance),
 * T17 (discharge:202 m_count[d]==1 && d<n), and T18 (push:166 limited
 * m_excess[target]>=m_limit). Per byte-equal-tracer "Extensive printout"
 * discipline.
 *
 * Behaviour-bit-equal to upstream when probes elide. Sibling header takes
 * precedence via -I instrumented/include in build.sh.
 *****************************************************************************/
...
class push_relabel {
 ...
 private:
    ...
    void push(NodeID source, EdgeID e, NodeID sourceDistance) {
        ...
        // T16: sourceDistance <= m_distance[target] (push_relabel.h:148).
        // Non-strict <=; equal-distance push BLOCKED.
        bool t16 = (sourceDistance <= m_distance[target]);
        std::fprintf(stderr,
            "[TRACE-PR-PUSH] src:%u e:%lu tgt:%u sdist:%u dist_tgt:%u "
            "t16_block:%d\n",
            source, (unsigned long)e, target, sourceDistance,
            m_distance[target], (int)t16);
        if (t16) [[likely]] return;
        ...
        std::fprintf(stderr,
            "[TRACE-PR-PUSH] src:%u tgt:%u cap:%ld flow:%ld excess:%ld "
            "amount:%ld\n",
            source, target, (long)capacity, (long)flow,
            (long)m_excess[source], (long)amount);
        if (amount == 0) return;
        ...
        if constexpr (limited) {
            // T18: m_excess[target] >= m_limit (push_relabel.h:166).
            bool t18 = (target == m_sink && m_excess[target] >= m_limit);
            std::fprintf(stderr,
                "[TRACE-PR-PUSH] limited tgt:%u excess_tgt:%ld limit:%ld "
                "t18_reached:%d\n",
                target, (long)m_excess[target], (long)m_limit, (int)t18);
            if (t18) {
                m_limitreached = true;
                return;
            }
    ...
    void discharge(NodeID node) {
        ...
        if (m_excess[node] > 0) {
            // T17: m_count[m_distance[node]] == 1 && m_distance[node] < n
            // (push_relabel.h:202). Non-strict ==.
            bool count_eq_1 = (m_count[m_distance[node]] == 1);
            bool dist_lt_n = (m_distance[node] < m_G->number_of_nodes());
            bool t17 = (count_eq_1 && dist_lt_n);
            std::fprintf(stderr,
                "[TRACE-PR-DC] node:%u dist:%u count:%d n:%u "
                "count_eq_1:%d dist_lt_n:%d t17_gap:%d\n",
                node, m_distance[node], m_count[m_distance[node]],
                m_G->number_of_nodes(),
                (int)count_eq_1, (int)dist_lt_n, (int)t17);
            if (t17) {
 ...
 public:
    ...
    std::pair<FlowType, std::vector<NodeID> > solve_max_flow_min_cut(
        ...
        std::fprintf(stderr,
            "[TRACE-PR] solve_entry G_n:%u curr_source:%u sources_n:%zu "
            "limit:%ld problem_id:%zu\n",
            G->n(), curr_source, sources.size(), (long)limit,
            problem_id);
        t.restart();
        ...
        while (!m_Q.empty()) {
            ...
            m_active[v] = false;
            std::fprintf(stderr,
                "[TRACE-PR] main_pop iter:%zu v:%u dist:%u excess:%ld\n",
                pr_iter, v, m_distance[v], (long)m_excess[v]);
            discharge(v);
        ...
        (void)depthPR;
        std::fprintf(stderr,
            "[TRACE-PR] solve_exit total_flow:%ld\n",
            (long)total_flow);

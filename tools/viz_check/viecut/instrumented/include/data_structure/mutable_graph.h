 ...
 public:
    ...
    void deleteEdge(NodeID node, EdgeID edge) {
        ...
        if (!vertices[node].size() || !vertices[target].size()) {
            LOG1 << "ERROR: Deleting edge from empty vertex! Exiting!";
            exit(3);
        }
    ...
    NodeID contractSparseTargetNoEdge(NodeID node, NodeID target) {
        if (node == target + 1) {
            // 2-element membership; iteration-order independent. Pass through
            // TracerSet to match contractVertexSet's TracerSet signature.
            TracerSet<NodeID> nodes = { node, target };
    ...
    NodeID contractEdge(NodeID node, EdgeID edge) {
        ...
        TracerMap<NodeID, std::tuple<EdgeWeight, EdgeID, EdgeID> > map;
    ...
    void contractVertexSet(const TracerSet<NodeID>& vertex_set) {
        ...
        TracerMap<NodeID, std::tuple<NodeID, EdgeID, EdgeID> > map;
    ...
    void internalDeleteEdge(NodeID n, EdgeID e) {
        if (vertices[n].size() > e + 1) {
            ...
            if (vertices[n].size()) {
                ...
                LOG1 << "ERROR: Trying to delete edge from empty vertex!";
                exit(-1);
            }
        }
    }

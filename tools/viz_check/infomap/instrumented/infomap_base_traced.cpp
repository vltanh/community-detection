...
// ──────────────────────────────────────────────────────────────────
// Tracer hook state. Records leaf->top-module membership + L at every
// stage boundary inside InfomapBase::partition. main_traced.cpp reads
// it via getInfomapTrace().
// ──────────────────────────────────────────────────────────────────
struct InfomapTraceStage {
  std::string label;            // "init" | "findTopModulesRepeatedly_<i>" | "fineTune_<i>" | "coarseTune_<i>" | "final"
  double L = 0.0;
  double L_index = 0.0;
  double L_module = 0.0;
...
static void traceCaptureStage(InfomapBase& self, const std::string& label)
{
  ...
  st.L_index = self.getIndexCodelength();
  st.L_module = self.getModuleCodelength();
  st.num_top_modules = self.numTopModules();

  // leaf_to_top: each leaf's TOP-level ancestor module index, sorted by
  // the canonical iteration order over root's children (== position in
  // root's child list). For final-stage capture (after consolidatedCluster
  // Index has run), this matches `im.getModules(1, false).second` per
  // leaf — equivalent to what canonical_run.py emits as the cluster_id
  // before drop-singletons + ASC-renumber post-processing.
  auto& root = self.root();
  std::map<InfoNode*, unsigned int> top_module_id;
  unsigned int next_top = 0;
  for (auto& topModule : root) {
    top_module_id[&topModule] = next_top++;
  }

  st.leaf_to_top.assign(self.numLeafNodes(), 0u);
  // Walk leaf-iterator (level 1) which canonical uses for two-level
  // output. The iterator emits leaves in the same order as
  // m_leafNodes vector (insertion order from input parsing).
  unsigned int leaf_idx = 0;
  for (auto it = self.iterLeafNodes(); !it.isEnd(); ++it) {
    InfoNode* leaf = &(*it);
    if (leaf == nullptr) { ++leaf_idx; continue; }
    // Walk up to find the immediate child-of-root ancestor (== top).
    InfoNode* top = leaf;
    while (top != nullptr && top->parent != nullptr
           && top->parent != &root) {
      top = top->parent;
    }
    auto found = (top != nullptr) ? top_module_id.find(top)
                                  : top_module_id.end();
    st.leaf_to_top[leaf_idx++] = (found != top_module_id.end())
        ? found->second : 0u;
  }
  g_infomap_trace.stages.push_back(std::move(st));
}
...
void InfomapBase::partition()
{
  ...
  traceCaptureStage(*this, "findTopModulesRepeatedly_0");
  ...
  while (numTopModules() > 1 && (m_tuneIterationIndex + 1) != tuneIterationLimit) {
    ...
    if (doFineTune) {
      ...
      traceCaptureStage(*this, std::string("fineTune_") + std::to_string(m_tuneIterationIndex));
      if (numEffectiveLoops > 0) {
        ...
        findTopModulesRepeatedly(levelAggregationLimit);
        traceCaptureStage(*this, std::string("findTopModulesRepeatedly_after_fineTune_") + std::to_string(m_tuneIterationIndex));
      } else {
      ...
      if (!noCoarseTune) {
        ...
        traceCaptureStage(*this, std::string("coarseTune_") + std::to_string(m_tuneIterationIndex));
        if (numEffectiveLoops > 0) {
          Log(1, 3) << "done in " << numEffectiveLoops << " effective loops to codelength " << *this << " in " << numTopModules() << " modules.\n";
          // Continue to merge fine-tuned modules
          findTopModulesRepeatedly(levelAggregationLimit);
          traceCaptureStage(*this, std::string("findTopModulesRepeatedly_after_coarseTune_") + std::to_string(m_tuneIterationIndex));
        } else {
          Log(1, 3) << "no improvement.\n";
        }
      }
  ...
  // [TRACE-IM] capture final partition state.
  g_infomap_trace.L_final = getCodelength();
  traceCaptureStage(*this, "final");
}

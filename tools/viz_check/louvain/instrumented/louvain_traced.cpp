...
// [TRACER FORK] Verbatim copy of externals/louvain/src/louvain.cpp +
// stderr/JSON trace injection in `Louvain::one_level` only. Every other
// line is unchanged from the canonical. This fork is built as
// /tmp/louvain_kernel_check together with the canonical's other .o
// files (graph_binary.o, modularity.o, etc) and a custom main_traced
// that emits the structured trace to stdout JSON.
#include "louvain.h"
#include <cstdio>
#include <vector>

struct LouvainTraceMove {
    int level;     // aggregation level (0 = base)
    int pass;      // sweep number within this level
    int visit;     // visit index in random_order
    int node;
    int from_comm;
    int to_comm;
    long double dQ;
...
struct LouvainTraceLevel {
    int level;
    int n_nodes;
    std::vector<int> random_order;     // random_order at start of one_level
...
void louvain_trace_begin_level(int n_nodes) {
    gCurrentLevel++;
    gTrace.push_back({});
...
Louvain::one_level() {
  ...
  // [TRACE-LV] Capture this level's random_order + initial quality.
  louvain_trace_begin_level(qual->size);
  gTrace.back().random_order = random_order;
  gTrace.back().quality_before = new_qual;
  fprintf(stderr, "[TRACE-LV] LEVEL_BEGIN level=%d n=%d Q=%.6Lf\n",
          gCurrentLevel, qual->size, new_qual);
  ...
  do {
	...
	if (increase>best_increase) {
      ...
      // [TRACE-LV] Record per-visit move (node, from_comm, to_comm, dQ).
      gTrace.back().moves.push_back({
        gCurrentLevel, nb_pass_done - 1, node_tmp,
        node, node_comm, best_comm,
        best_increase, best_comm != node_comm
      });
  ...
  // [TRACE-LV] Capture final quality + pass count for this level.
  gTrace.back().quality_after = new_qual;
  gTrace.back().passes = nb_pass_done;
  fprintf(stderr, "[TRACE-LV] LEVEL_END level=%d passes=%d Q=%.6Lf -> %.6Lf moves=%zu\n",
          gCurrentLevel, nb_pass_done, gTrace.back().quality_before, new_qual,
          gTrace.back().moves.size());

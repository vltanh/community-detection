// [TRACER FORK] Verbatim copy of libleidenalg/src/MutableVertexPartition.cpp +
// trace injection inside `move_node` (admin update tuple per move) and
// `cache_neigh_communities` (per-edge accumulator stream). Other functions
// (init_admin, renumber_communities, rank_order_communities, etc.) are
// untouched. Probe emission is env-gated via _ld_probes_enabled() so the
// default L4 stress sweep is unaffected.
//
// Closes audit row M (move_node admin update order, MutableVertexPartition.
// cpp:540-736) and row H + I crossover on the cache path (MutableVertex
// Partition.cpp:799-867).
...
// [TRACE-LD-MV] / [TRACE-LD-CACHE] probe gate. Single source of truth in
// optimiser_traced.cpp (LEIDEN_DUMP_PROBES env). Declared extern so this
// translation unit picks up the same in-process cached flag.
extern bool _ld_probes_enabled();
static void _ld_mv_emit_hex(double x) {
...
vector<size_t> MutableVertexPartition::rank_order_communities(vector<MutableVertexPartition*> partitions)
{
  ...
  // [TRACE-LD-RANK] PRE: dump per-community (csize, cnodes) tuples in the
  // input order (id ASC). Closes P0 #14 (top P1 #14 promoted to P0 per
  // gap audit): cpp tie-break is csize-DESC, cnodes-DESC, id-ASC and the
  // sort output `new_comm_id[]` was previously visible only via post-
  // renumber membership. Probe at MutableVertexPartition.cpp:404
  // (rank_order_communities sort site).
  if (_ld_probes_enabled()) {
    fprintf(stderr, "[TRACE-LD-RANK] PRE nb_comms=%zu rows=", nb_comms);
    for (size_t i = 0; i < nb_comms; i++) {
      fprintf(stderr, "%s(%zu,%zu,%zu)", i ? "," : "",
              csizes[i][0], csizes[i][1], csizes[i][2]);
    }
    fprintf(stderr, "\n");
  }
  ...
  // [TRACE-LD-RANK] POST: dump new_comm_id mapping (old_comm -> new_id).
  if (_ld_probes_enabled()) {
    fprintf(stderr, "[TRACE-LD-RANK] POST nb_comms=%zu remap=", nb_comms);
    for (size_t i = 0; i < nb_comms; i++) {
      fprintf(stderr, "%s%zu", i ? "," : "", new_comm_id[i]);
    }
    fprintf(stderr, "\n");
  }
...
{
  // [TRACE-LD-ADDEMPTY] PRE: n_communities before grow. Closes P0 #17
  // (cpp:766-784 add_empty_community branch result). Probe at function
  // entry so `(n_comms_before, new_comm, n_comms_after, empties_size)`
  // form a contiguous tuple.
  size_t _ld_n_before = this->_n_communities;
  ...
  #endif
  if (_ld_probes_enabled()) {
    fprintf(stderr, "[TRACE-LD-ADDEMPTY] n_before=%zu new_comm=%zu n_after=%zu empties_size=%zu\n",
            _ld_n_before, new_comm, this->_n_communities, this->_empty_communities.size());
  }
  return new_comm;
}
...
void MutableVertexPartition::move_node(size_t v,size_t new_comm)
{
  ...
  #endif
  // [TRACE-LD-MV] ENTRY: per-`move_node` admin pre-state tuple. Closes
  // audit row M (lines 540-736 all silent in production tracer). Probes
  // every input scalar + every admin accumulator pre-update.
  if (_ld_probes_enabled() && new_comm != old_comm) {
    fprintf(stderr, "[TRACE-LD-MV] ENTRY v=%zu old=%zu new=%zu nsize=", v, old_comm, new_comm);
    _ld_mv_emit_hex(node_size);
    fprintf(stderr, " csize_old=");
    _ld_mv_emit_hex(this->_csize[old_comm]);
    fprintf(stderr, " csize_new=");
    _ld_mv_emit_hex(this->_csize[new_comm]);
    fprintf(stderr, " cnodes_old=%zu cnodes_new=%zu tw_in_old=",
            this->_cnodes[old_comm], this->_cnodes[new_comm]);
    _ld_mv_emit_hex(this->_total_weight_in_comm[old_comm]);
    fprintf(stderr, " tw_in_new=");
    _ld_mv_emit_hex(this->_total_weight_in_comm[new_comm]);
    fprintf(stderr, " tw_from_old=");
    _ld_mv_emit_hex(this->_total_weight_from_comm[old_comm]);
    fprintf(stderr, " tw_from_new=");
    _ld_mv_emit_hex(this->_total_weight_from_comm[new_comm]);
    fprintf(stderr, " tw_to_old=");
    _ld_mv_emit_hex(this->_total_weight_to_comm[old_comm]);
    fprintf(stderr, " tw_to_new=");
    _ld_mv_emit_hex(this->_total_weight_to_comm[new_comm]);
    fprintf(stderr, " tw_all=");
    _ld_mv_emit_hex(this->_total_weight_in_all_comms);
    fprintf(stderr, " tpe_all=");
    _ld_mv_emit_hex(this->_total_possible_edges_in_all_comms);
    fprintf(stderr, " directed=%d\n", this->graph->is_directed() ? 1 : 0);
  }
  ...
  if (new_comm != old_comm)
  {
    ...
    // [TRACE-LD-MV] DELTA_PE: the analytic-delta formula from
    // MutableVertexPartition.cpp:572. Closes top P0 #1 (FMA-fusion
    // candidate; `-ffp-contract=off` essential, this probe verifies).
    if (_ld_probes_enabled()) {
      fprintf(stderr, "[TRACE-LD-MV] DELTA_PE v=%zu delta_pe=", v);
      _ld_mv_emit_hex(delta_possible_edges_in_comms);
      fprintf(stderr, "\n");
    }
  ...
  if (this->_cnodes[old_comm] == 0)
  {
    ...
    #endif
    // [TRACE-LD-EMPTY] ADD: dump empties list BEFORE push_back. Closes
    // P0 #8 (audit: cpp:608-622 LIFO order critical for byte-equality).
    if (_ld_probes_enabled()) {
      fprintf(stderr, "[TRACE-LD-EMPTY] ADD v=%zu c=%zu pre_size=%zu pre_list=",
              v, old_comm, this->_empty_communities.size());
      for (size_t i = 0; i < this->_empty_communities.size(); i++) {
        fprintf(stderr, "%s%zu", i ? "," : "", this->_empty_communities[i]);
      }
      fprintf(stderr, "\n");
    }
  ...
  if (this->_cnodes[new_comm] == 0)
  {
    ...
    #endif
    // [TRACE-LD-EMPTY] ERASE_BEGIN: dump empties pre-erase.
    if (_ld_probes_enabled()) {
      fprintf(stderr, "[TRACE-LD-EMPTY] ERASE_PRE v=%zu c=%zu pre_size=%zu pre_list=",
              v, new_comm, this->_empty_communities.size());
      for (size_t i = 0; i < this->_empty_communities.size(); i++) {
        fprintf(stderr, "%s%zu", i ? "," : "", this->_empty_communities[i]);
      }
      fprintf(stderr, "\n");
    ...
    // [TRACE-LD-EMPTY] ERASE_POST: dump empties post-erase.
    if (_ld_probes_enabled()) {
      fprintf(stderr, "[TRACE-LD-EMPTY] ERASE_POST v=%zu c=%zu post_size=%zu post_list=",
              v, new_comm, this->_empty_communities.size());
      for (size_t i = 0; i < this->_empty_communities.size(); i++) {
        fprintf(stderr, "%s%zu", i ? "," : "", this->_empty_communities[i]);
      }
  ...
  for (size_t mode_i = 0; mode_i < 2; mode_i++)
  {
    ...
    for (size_t idx = 0; idx < degree; idx++)
    {
      ...
      // [TRACE-LD-MV] EDGE: per-edge contribution. Closes top P0 #2 + #3
      // + #4. cpp:669-684 (`_total_weight_from/to_comm` update),
      // cpp:695 (`int_weight = w/(2-directed)/(u==v?2:1)`),
      // cpp:700-720 (`_total_weight_in_comm` + `_total_weight_in_all_comms`
      // update). Probe fires AFTER from/to update and BEFORE in-comm update
      // so the running accumulator snapshot is mid-edge.
      bool intra_old = (old_comm == u_comm);
      bool intra_new = ((new_comm == u_comm) || (u == v));
      if (_ld_probes_enabled()) {
        fprintf(stderr, "[TRACE-LD-MV] EDGE v=%zu u=%zu e=%zu mode=%d u_comm=%zu w=",
                v, u, e, (int)mode, u_comm);
        _ld_mv_emit_hex(w);
        fprintf(stderr, " int_weight=");
        _ld_mv_emit_hex(int_weight);
        fprintf(stderr, " intra_old=%d intra_new=%d tw_from_old=",
                intra_old ? 1 : 0, intra_new ? 1 : 0);
        _ld_mv_emit_hex(this->_total_weight_from_comm[old_comm]);
        fprintf(stderr, " tw_from_new=");
        _ld_mv_emit_hex(this->_total_weight_from_comm[new_comm]);
        fprintf(stderr, " tw_to_old=");
  ...
  #endif
  // [TRACE-LD-MV] EXIT: post-update admin tuple. Bookend matches ENTRY
  // so a single diff between cpp + JS pinpoints which accumulator drifted.
  if (_ld_probes_enabled() && new_comm != old_comm) {
    fprintf(stderr, "[TRACE-LD-MV] EXIT v=%zu old=%zu new=%zu csize_old=", v, old_comm, new_comm);
    _ld_mv_emit_hex(this->_csize[old_comm]);
    fprintf(stderr, " csize_new=");
    _ld_mv_emit_hex(this->_csize[new_comm]);
    fprintf(stderr, " cnodes_old=%zu cnodes_new=%zu tw_in_old=",
    ...
    fprintf(stderr, " tpe_all=");
    _ld_mv_emit_hex(this->_total_possible_edges_in_all_comms);
    fprintf(stderr, " n_empty=%zu\n", this->_empty_communities.size());
  }
...
void MutableVertexPartition::cache_neigh_communities(size_t v, igraph_neimode_t mode)
{
  ...
  // [TRACE-LD-CACHE] CACHE_BEGIN. Closes top P0 #9 + #5 entry boundary.
  // cache_neigh_communities (MutableVertexPartition.cpp:799-867) is the
  // accumulator producer for `weight_to_comm`/`weight_from_comm` used by
  // every diff_move; its per-edge updates were entirely silent.
  if (_ld_probes_enabled()) {
    fprintf(stderr, "[TRACE-LD-CACHE] BEGIN v=%zu mode=%d degree=%zu\n",
            v, (int)mode, degree);
  }
  for (size_t idx = 0; idx < degree; idx++)
  {
    ...
    bool selfLoopHalved = (u == v && !this->graph->is_directed());
    if (selfLoopHalved)
        w /= 2.0;
    #ifdef DEBUG
      cerr << "\t" << "Edge (" << v << "-" << u << "), Comm (" << v_comm << "-" << comm << ") weight: " << w << "." << endl;
    #endif
    // [TRACE-LD-CACHE] EDGE. Closes top P0 #5 (self-loop halve site at
    // cpp:851-852) + #9 (per-edge `_cached_weight_tofrom_community[comm] +=
    // w` accumulator update at cpp:856). Probe fires BEFORE the += so
    // the pre/post pair is fully visible.
    if (_ld_probes_enabled()) {
      double pre = (*_cached_weight_tofrom_community)[comm];
      fprintf(stderr, "[TRACE-LD-CACHE] EDGE v=%zu idx=%zu u=%zu e=%zu comm=%zu w=",
              v, idx, u, e, comm);
      _ld_mv_emit_hex(w);
      fprintf(stderr, " self_halved=%d pre=", selfLoopHalved ? 1 : 0);
      _ld_mv_emit_hex(pre);
      fprintf(stderr, " post=");
      _ld_mv_emit_hex(pre + w);
      fprintf(stderr, "\n");
    }
  ...
  // [TRACE-LD-CACHE] END. Bookend so the per-call accumulator stream has
  // an obvious closing marker.
  if (_ld_probes_enabled()) {
    fprintf(stderr, "[TRACE-LD-CACHE] END v=%zu mode=%d ncached=%zu\n",
            v, (int)mode, _cached_neighs_comms->size());
  }
  #ifdef DEBUG
